...
class Discriminator(Model):
    ...
    def __init__(self, filter_multiplier):
        ...
        self.conv1 = tf.keras.layers.Conv2D(8*filter_multiplier,                        #64
                                            kernel_size=4, 
                                            strides=1,                              
                                            padding='same', 
                                            kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.02), 
        ...
        self.conv2 = tf.keras.layers.Conv2D(1,                                          #64
                                            kernel_size=4, 
                                            strides=1, 
                                            padding='same', 
                                            kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.02), 
    ...
    def build_graph(self):
        ...
        x3 = Input(shape=(512, 512, 1))
        return Model(inputs=[x1, x2, x3], outputs=self.call([x1, x2, x3]), name='Discriminator')
    ...
    def visualise_progress(self, epoch, generator):
        ...
        if epoch % self.frequency == 0:
            ...
            for step in range(len(self.val_data)):
                ...
                record = [input, mask, output, target]
                for r in record: 
                    progress.append(r)    
            ...
            fig = plt.figure(figsize=(5.12*width, 5.12*height), dpi=100)
...
class GAN(Model):
    ...
    def g_loss_function(self, d_g_output):
        ...
        L_G = -1 * tf.math.reduce_mean(d_g_output)                                           
        return L_G

    #def g_loss_function(self, d_g_output):
    #    L_G = tf.math.reduce_mean(tf.nn.relu(1.0 - d_g_output))                #Target predictions  1.   
    #    return L_G                                           

    def d_loss_function(self, d_g_output, d_d_output):
        L_D  = tf.math.reduce_mean(tf.nn.relu(1.0 + d_g_output))                #Target predictions -1.   
        L_D += tf.math.reduce_mean(tf.nn.relu(1.0 - d_d_output))                #Target predictions  1.   
        return L_D                                   
    ...
    def gan_train(self, epoch):
        ...
        for step in range(0, len(self.gan_data), 1):
            ...
            input_images, mask_images, target_images  = self.gan_data[step]

            with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape:

                g_g_output = self.generator([input_images, mask_images])

                d_g_output = self.discriminator([input_images, mask_images, g_g_output])
                d_d_output = self.discriminator([input_images, mask_images, target_images])                     
                
                g_loss = self.g_loss_function(d_g_output)
                d_loss = self.d_loss_function(d_g_output, d_d_output)
            ...
            self.g_acc.update_state(tf.ones_like(d_g_output), d_g_output)
            self.d_acc.update_state(tf.ones_like(d_d_output), d_d_output)
    ...
    def gan_fit(self, 
        ...
        for epoch in range(epoch_first, epoch_last+1, 1):
            ...
            self.g_progress.visualise_progress(epoch, self.generator, self.augmenter)
...
gan_data = GAN_Data(dataframe='/path/TrainDf.csv', 
                    batch_size=batch_size)

val_data = GAN_Data(dataframe='/path/ValidDf.csv', 
                    batch_size=1)

g_progress = Progress_Visualiser(save='/path/progress/',
                                 val_data=val_data,
                                 frequency=1)
model.gan_fit(epoch_first=1,
              ...
              log_df_path='/path/log.csv',
              save_directory='/path/checkpoint/',
              save_maxnumber=16,
              save_frequency=1,
              save_restoring=None)
              #save_restoring='/path/checkpoint/ckpt-1') #Path to the desired checkpoint.
